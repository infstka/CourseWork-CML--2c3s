...
// Select button
const acquista = document.querySelector(".btnBuy");
// Select film selector
let film = document.getElementById("film");
// Select sedie
const sedie = document.querySelectorAll(".sedia:not(.occupated)");
// Global Variables
...
rowNumb.forEach(element => {
  element.innerHTML = `Ряд ${element.id}`;
});
...
const loadData = () => {
  ...
  let occupate = JSON.parse(localStorage.getItem("occupated"));
...
const sedieReload = document.querySelectorAll(".sedia:not(.occupated");
...
acquista.addEventListener("click", () => {
  ...
  sedia.forEach(element => {
    element.classList.remove("selezionata");
    element.classList.add("occupated");
    // Clear all fields
    element.innerHTML = "";
    ...
    let seatBusySelec = document.querySelectorAll(".row .sedia.occupated");
    ...
    // Save
    localStorage.setItem("occupated", JSON.stringify(localStorageSeatsOccupied));
  });
});